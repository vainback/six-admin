...
import { Organization, reqOrgs } from '@/api/system/organization';
...
const form = ref<Organization>({ ...props.formData }); // 深拷贝 防止与列表数据双向绑定
const emit = defineEmits(['close', 'refresh', 'closeAndRefresh']);

const submit = async () => {
    try {
        await reqOrgs(props.isEdit ? 'save' : 'add', { model: form.value });
