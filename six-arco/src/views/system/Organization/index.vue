                        ...
                        <template #cell="{ record }">
                            <a-space>
                                ...
                                <a-button
                                    ...
                                    :size="size"
                                    @click="toEdit(record)"
                                    v-permission="'system:organization:save'"
                                    >编辑
                                </a-button>
                                <a-popconfirm
                                    content="确认删除？"
                                ...
                                >
                                    <a-button
                                        ...
                                        status="danger"
                                        :size="size"
                                        v-permission="'system:organization:del'"
                                        >删除
                                    </a-button>
                                </a-popconfirm>
                            </a-space>
...
<script setup lang="ts">
    ...
    import { defaultRequestParam, OrderBy, RequestParam } from '@/api/base';
    ...
    import FormSave from '@/views/system/Organization/components/form-save.vue';
    import { Message } from '@arco-design/web-vue';
    import dayjs from 'dayjs';

    const size = ref(import.meta.env.VITE_STYLE_SIZE);
    onMounted(() => {
        query.value = defaultRequestParam({ ...EmptyOrganization });
    ...
    const query = ref<RequestParam<Organization>>({
        is_delete: false,
        order_by: [{ field: '`id`', is_desc: false }],
        model: { ...EmptyOrganization },
    });

    const list = ref<TreeOrganization[]>([]);
    const queryList = async () => {
        try {
            ...
            query.value.end_time = timePicker.value[1]
                ...
                : '';
            const res = await reqOrgs('tree-select', query.value);
            list.value = res.data || [];
        } catch (e) {
        } finally {
    ...
    const toDel = async (id: number) => {
        try {
            await reqOrgs('del', { model: { id: id } as Organization });
